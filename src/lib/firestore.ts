...
export const addQuestion = async (data: Omit<Question, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> => {
  const dbData: any = {
    ...
    is_locked: data.isLocked ?? true, // Default to true if not provided
  };
  ...
  switch (data.questionType) {
    case 'mcq':
      ...
      break;
    case 'true_false':
      dbData.options = (data as TrueFalseQuestion).options; // Should be [{id: 'true', text: '...'}, {id: 'false', text: '...'}]
    ...
    default:
      ...
      // Potentially throw an error or handle as a generic question
      break;
  }
...
export const addSubjectSection = async (subjectId: string, data: Omit<SubjectSection, 'id' | 'subjectId' | 'created_at' | 'updated_at' | 'lessons' | 'isLocked'>): Promise<string> => {
  // The 'id' for subject_sections is now assumed to be a UUID generated by the database
  ...
  // is_locked will default to true in the DB if not provided (as per your schema)
  ...
  return String(insertedData.id); // Return the UUID as a string
};
...
export const getSubjectSections = async (subjectId: string): Promise<SubjectSection[]> => {
  ...
  return data.map((section: any) => ({
    id: String(section.id), // id is now UUID, ensure it's string
...
export const deleteSubjectSection = async (subjectId: string, sectionId: string): Promise<void> => {
  const { error } = await supabase
    .from('subject_sections')
    .delete()
    .eq('id', sectionId); // 'id' is the UUID primary key
